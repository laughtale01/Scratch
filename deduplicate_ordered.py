...
# Read original order from 0b24f38
with open('player_0b24f38.js', 'r', encoding='utf-8') as f:
...
print("Extracting original order with comments...")

# Extract original order with comments
original_categories = {}
...
for cat_name in category_names:
    ...
    if match:
        ...
        print(f"  {cat_name}: {len(blocks_in_order)} blocks")
...
for cat, blocks in deduplicated_categories.items():
    original_count = len(original_categories[cat])
    new_count = len(blocks)
    print(f"  {cat}: {original_count} -> {new_count} ({new_count - original_count:+d})")

# Format with comments
def format_with_comments(cat_name, blocks):
    ...
    try:
        ...
        for cat_name in category_names:
            if cat_name in deduplicated_categories:
                ...
                new_array = format_with_comments(cat_name, blocks)
