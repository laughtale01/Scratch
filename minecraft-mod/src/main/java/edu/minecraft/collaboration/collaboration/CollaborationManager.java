...
public final class CollaborationManager {
    ...
    public boolean approveVisitRequest(UUID requestId, net.minecraft.server.MinecraftServer server) {
        ...
        if (request != null && request.getStatus() == VisitRequest.VisitStatus.PENDING) {
            ...
            if (requester != null && host != null) {
                ...
                // Notify both players
                notifyPlayer(requester, "You have been teleported to " + request.getHostName() + "'s world.");
                notifyPlayer(host, request.getRequesterName() + " has visited your world.");
    ...
    public boolean returnPlayerHome(ServerPlayer player) {
        ...
        if (homePos != null) {
            ...
            notifyPlayer(player, "You have returned to spawn point.");
            return true;
        }
    }
    ...
    public boolean emergencyReturnPlayer(ServerPlayer player) {
        ...
        if (returned) {
            ...
            // Notify player
            notifyPlayer(player, "Emergency return completed. Health and hunger restored.");
    ...
    public void notifyPlayer(ServerPlayer player, String message) {
        if (player != null) {
            player.sendSystemMessage(Component.literal("[Collaboration] " + message));
        }
    }
    
    public void broadcastToAllPlayers(net.minecraft.server.MinecraftServer server, String message) {
        if (server != null) {
            Component msg = Component.literal("[Collaboration] " + message);
