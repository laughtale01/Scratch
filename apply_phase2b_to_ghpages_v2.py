...
print('='*80)
print('フェーズ2B: gh-pagesブランチへの適用 (v2)')
print('='*80)
...
def add_getPlayerFacing_block(content):
    ...
    # Pattern to find getPosition block (using simpler text pattern)
    pattern = r"(\{\s*opcode:\s*'getPosition',\s*blockType:\s*'reporter',\s*text:\s*'プレイヤーの位置\s*\[COORD\]'[^}]+\}[^}]+\}),(\s*'---')"

    # If Japanese pattern doesn't match, try English pattern
    if not re.search(pattern, content):
        pattern = r"(\{\s*opcode:\s*'getPosition',\s*blockType:\s*'reporter',\s*text:\s*'\[COORD\]'[^}]+\}[^}]+\}),(\s*'---')"
...
def add_getBlockType_block(content):
    ...
    pattern = r"(\{\s*opcode:\s*'getPlayerFacing',\s*blockType:\s*'reporter',\s*text:\s*'プレイヤーの向き'\s*\}),(\s*'---')"
...
def add_setGameRule_block(content):
    ...
    pattern = r"(\{\s*opcode:\s*'setTime',\s*blockType:\s*'command'[^}]+menu:\s*'timeValues'[^}]+\}[^}]+\}),(\s*'---')"
...
def add_getPlayerFacing_method(content):
    ...
    # Pattern to find end of getPosition method - simplified
    pattern = r"(getPosition\s*\([^)]*\)\s*\{[^}]*sendCommandWithResponse\s*\(\s*'getPosition'[^}]+\}[\s\S]{0,500}?\}\s*;?\s*\})"
...
def add_getBlockType_method(content):
    ...
    pattern = r"(getPlayerFacing\s*\(\s*\)\s*\{[^}]*sendCommandWithResponse\s*\(\s*'getPlayerFacing'[^}]+\}[\s\S]{0,500}?\}\s*;?\s*\})"
...
def add_setGameRule_method(content):
    ...
    pattern = r"(setTime\s*\([^)]*\)\s*\{[^}]*sendCommand\s*\(\s*'setTime'[^}]+\}\s*\)\s*;?\s*\})"
